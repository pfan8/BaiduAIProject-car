...
def build_dataset():
    ...
    '''
    # 载入DataFrame
    if os.path.isfile(config.train_x_path) \
        and os.path.isfile(config.train_y_path) \
        and os.path.isfile(config.test_x_path):
        train_X = np.loadtxt(config.train_x_path)
        train_Y = np.loadtxt(config.train_y_path)
        test_X = np.loadtxt(config.test_x_path)
    else:
        print('Constructing DataSet...Wait for minutes')
